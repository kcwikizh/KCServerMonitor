...
import moe.kcwiki.swfunpacker.comparator.src.VerifyScr;
...
public class Controller {
    ...
    private final moe.kcwiki.swfunpacker.comparator.image.Controller verify;
    ...
    public Controller() throws IOException{
        ...
        ffdec=new UnpackSwf();
        verify=new moe.kcwiki.swfunpacker.comparator.image.Controller();
    }
    ...
    public void Analysis(String filename,String filepath,String sourcepath) throws InterruptedException, Exception{
        ...
        if(filename.contains("Core")){
            if(!new moe.kcwiki.swfunpacker.coredecryptor.CoreRecover().unlockCore(filepath+File.separator+filename, MainServer.getTempFolder())){msgPublish.msgPublisher("Core.swf 解码失败。",0,-1);}
            ffdec.ffdec(outputpath,MainServer.getTempFolder()+File.separator+"Core_hack.swf");
            new moe.kcwiki.swfunpacker.coredecryptor.CoreDecrypt().getData(outputpath+File.separator+Server.getCoremap(), outputpath+File.separator+Server.getCoresound());
